...
define不是常量，不会分配内存。const的内存分配在只读数据段的常量区
...
static将数据保留在数据段中静态区，表示在程序运行中不可以释放。const将数据保存在只读数据段中常量区，表示在程序运行中不可以更改。如果const static一起用，那就表示在程序运行中不可释放也不可改变，会存在？。
