...
这里涉及到引用折叠规则，对照折叠规则可以很容易发现只有使用&&才能够满足"保持它传进来前的性质"的目的

使用const&或者是&来都会改变进来之前的性质，const&会导致不能改动，&会导致在处理传入右值模板(T&&)的时候也变为左值，这些都不满足想达到"保持它传进来前的性质"的目的，因此使用的是&&

以下是引用折叠的规则，简记为“有左都变左”
